...
# Encoding: integer tensors represent genotype (see paper one-hot view).
...
class GanAlgorithm():
    ...
    def search(self):
        # Return a new generator genotype
    ...
    def search_constraint(self):
        # Return a new generator genotype under FLOPs/param constraints
    ...
    def search_dis(self):
        # Return a new discriminator genotype
    ...
    def sample(self):
        ...
        for i in range(self.steps):         # 3 cell
            ...
            while(genotype[i][2] == 0 and genotype[i][3] == 0):  # unset; resample
                for k in range(2):
                    genotype[i][k+2] = random.randint(0, 6)
            while(genotype[i][4] == 0 and genotype[i][5] == 0 and genotype[i][6] == 0):  # all 'none' on edges 4-6; resample
                for k in range(2, self.normal_nodes):           # 2 - 5
                    genotype[i][k+2] = random.randint(0, 6)     # at least one non-zero edge
        return genotype
    ...
    def sample_constraint(self):
        ...
        while (cnt == 0):
            ...
            if (FlopsG >= 1500 and FlopsG <= 3000) and (param_szie >= 6.5 and param_szie <= 15) and (param_cell_szie[0] <= self.args.size) and (param_cell_szie[1] <= self.args.size) and (param_cell_szie[2] <= self.args.size):
                if (param_cell_szie[0] > 0.5) and (param_cell_szie[1] > 0.5) and (param_cell_szie[2] > 0.5): #1
    ...
    def sample_D(self):
        ...
        for i in range(self.steps):
            genotype[i][0] = random.randint(1, 6)                       # head block
            while(genotype[i][1] == 0 and genotype[i][2] == 0):
                for k in range(2):
                    genotype[i][k+2] = random.randint(0, 6)
            while(genotype[i][3] == 0 and genotype[i][4] == 0):         # middle
                for k in range(2, 4):
                    genotype[i][k+1] = random.randint(0, 6)
            genotype[i][self.dis_normal_nodes] = random.randint(-1, 5)  # tail: downsampling op
            if genotype[i][self.dis_normal_nodes] == -1:
                genotype[i][self.dis_normal_nodes+1] = -1               # paired with -1 (no extra op)
            else:
                genotype[i][self.dis_normal_nodes+1] = random.randint(0, 5)   # downsampling variant
        return genotype
