...
def count_cosine_similarity_6layer(FID_arch_Params):
    A = np.array([ 0.8, 0.8, 0.8, 1.0, 0.7, 1.1])  # values in [0, 6]
    scaled_A = np.array([(x / 6) * 100 for x in A])  # scale to [0, 100]
    B = FID_arch_Params
    scaled_B = np.array([(x / 6) * 100 for x in B])  # scale to [0, 100]
    # Cosine similarity
...
def count_cosine_similarity_3cell(FID_arch_Params):
    A = np.array([1.5, 1.8, 1.8])  # values in [0, 6]
    scaled_A = np.array([(x / 6) * 100 for x in A])  # scale to [0, 100]
    B = FID_arch_Params
    scaled_B = np.array([(x / 6) * 100 for x in B])  # scale to [0, 100]
    # Cosine similarity
...
def calculate_similarity_score(reference_value_min, unknown_value, reference_value_max):
    if reference_value_min == unknown_value:
        ...
        return 1  # unknown below reference_min -> max similarity
