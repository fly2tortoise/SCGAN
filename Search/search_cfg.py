...
def parse_args():
    ...
    parser.add_argument('--num_individual', type=int, default=10, help='numbers of  EA individual')
    ...
    parser.add_argument('--size', type=float, default=2.2, help='constraint size (per-cell param cap in search_algs_constraint)')
    # Prefer_cluster_MOEAD: pick cluster_select architectures by hierarchical clustering on the middle-ranked slice; must be < offspring count N (typically num_individual)
    parser.add_argument('--cluster_select', type=int, default=8, help='number of cluster-diverse picks in MOEAD+cluster selection')
