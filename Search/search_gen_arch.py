...
def main():
    ...
    # Genotype search space (GanAlgorithm)
    gan_alg = GanAlgorithm(args)

    # Generator super-network (weights shared; genotype selects subgraph)
    ...
    # IS/FID via PyTorch (replaces legacy TF 1.x eval)
    ...
    # Search generator architectures
    for epoch in tqdm(range(int(start_epoch), int(args.epoch_generator)), desc='search generator'):
