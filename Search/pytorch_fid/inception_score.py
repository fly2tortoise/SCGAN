...
# PyTorch IS (slow ~49s); IS_max ~9.6 vs ~11.31 mean in reference setup
...
def inception_score(imgs, cuda=True, batch_size=32, resize=True, splits=10):
    ...
    """
    # list -> numpy; dataset items may be (3,H,W) torch; numpy uses (H,W,3)
    t1 = time.time()
    ...
    imgs_tensor = torch.from_numpy(imgs_np.transpose((0, 3, 1, 2)))
    imgs = imgs_tensor
